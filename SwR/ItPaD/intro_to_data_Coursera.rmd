...
sfo_feb_flights %>%
  summarise(mean_ad = mean(arr_delay), median_ad = median(arr_delay), sd_ad = sd(arr_delay), n = n())
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(IQR_ad = IQR(arr_delay)) %>%
  arrange(desc(IQR_ad))
```
...
ggplot(nycflights, aes(y = avg_speed, x = distance)) +
  geom_point()
```
